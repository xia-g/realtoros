"""
Knowledge Governance — API integration tests.

Tests GET /knowledge/governance/check/{revision_id}.
"""
...
def _snap(nodes=(), edges=(), links=()):
    nodes_t = tuple(nodes) if isinstance(nodes, tuple) else (nodes,)
    edges_t = tuple(edges) if isinstance(edges, tuple) else (edges,)
    links_t = tuple(links) if isinstance(links, tuple) else (links,)
    return KnowledgeSnapshot(
        graph=KnowledgeGraph(nodes=nodes_t, edges=edges_t),
        provenance=KnowledgeProvenance(provenance_id=ProvenanceId.generate(),
                                        chain=ProvenanceChain(links=links_t)),
...
def app():
    ...
    valid = KnowledgeRevision(
        revision_id=KnowledgeRevisionId(value="gov-valid"),
    ...
    broken = KnowledgeRevision(
        revision_id=KnowledgeRevisionId(value="gov-broken"),
...
class TestGovernanceAPI:
    ...
    def test_valid_approved(self, client):
        r = client.get("/api/v1/knowledge/governance/check/gov-valid")
        assert r.status_code == 200
        assert r.json()["decision"]["status"] == "APPROVED"

    def test_broken_rejected(self, client):
        r = client.get("/api/v1/knowledge/governance/check/gov-broken")
        assert r.status_code == 200
        assert r.json()["decision"]["status"] == "REJECTED"

    def test_nonexistent_404(self, client):
        r = client.get("/api/v1/knowledge/governance/check/nonexistent")
        assert r.status_code == 404

    def test_latest_endpoint(self, client):
        r = client.get("/api/v1/knowledge/governance/check/latest")
        assert r.status_code == 200
        assert r.json()["decision"]["status"] in ("REJECTED", "FLAGGED")

    def test_deterministic(self, client):
        r1 = client.get("/api/v1/knowledge/governance/check/gov-valid")
        r2 = client.get("/api/v1/knowledge/governance/check/gov-valid")
        assert r1.json()["decision"]["status"] == r2.json()["decision"]["status"]

    def test_reason_present(self, client):
        r = client.get("/api/v1/knowledge/governance/check/gov-broken")
        data = r.json()["decision"]
