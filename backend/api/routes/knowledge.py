...
async def knowledge_stats(session: AsyncSession = Depends(get_session)):
    ...
    nodes = 0; edges = 0; embeddings = 0; chunks = 0
    
    result = await session.execute(select(func.count(GraphNode.id)))
    nodes = result.scalar() or 0
    result = await session.execute(select(func.count(GraphEdge.id)))
