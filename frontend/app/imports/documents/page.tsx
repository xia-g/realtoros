...
export default function DocumentsImportPage() {
  ...
  useEffect(() => {
    ...
    if (dealParam) {
      ...
      pendingDealRef.current = dealParam
    }
  }, [])
  ...
  const doUpload = async () => {
    ...
    try {
      ...
      const msg = e.message || 'Ошибка при обработке документа'
      setError(msg)
      setStep('select')
    }
  }
    ...
    for (let i = 0; i < maxPolls; i++) {
      ...
      try {
        ...
        if (data.status === 'unknown' || data.status === 'failed') {
          // OCR node unreachable — show clear warning
          throw new Error(data.error || 'OCR Node (GPU) недоступен. Распознавание временно не работает.')
        }
            ...
            <div className="bg-white rounded-xl border p-6">
              ...
              <ol className="text-sm space-y-3 text-gray-600">
                ...
                <li className="flex gap-2">
                  ...
                  <span>OCR (Tesseract) распознаёт текст</span>
                </li>
                <li className="flex gap-2">
